...
<section class="bg-gradient-to-br from-blue-900 to-cyan-800 text-white py-16">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <p class="text-blue-100">{{ __('site.booking.subtitle') }}</p>
    </div>
</section>
        ...
        <form method="POST" action="{{ route(app()->getLocale() === 'ar' ? 'ar.booking.store' : 'booking.store') }}"
            ...
            @if($packages->isNotEmpty())
                <div>
                    ...
                    <select name="package_id"
                            class="w-full px-4 py-2.5 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white">
            ...
            @else
                <div>
                    ...
                    <input type="text" name="test_name" value="{{ old('test_name') }}"
                           ...
                           class="w-full px-4 py-2.5 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>
            @endif
            ...
            <div>
                ...
                <input type="date" name="preferred_date" value="{{ old('preferred_date') }}"
                       ...
                       class="w-full px-4 py-2.5 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('preferred_date') border-red-400 @enderror">
            ...
            <div>
                ...
                <textarea name="notes" rows="3"
                          class="w-full px-4 py-2.5 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('notes') }}</textarea>
            </div>

            <button type="submit"
                    class="w-full py-3.5 bg-blue-700 hover:bg-blue-800 text-white font-bold rounded-xl transition-colors text-base">
                {{ __('site.booking.submit') }}
            </button>

            <p class="text-center text-xs text-slate-400">
                {{ __('site.booking.hint') }}:
                <a href="tel:19000" class="text-blue-600 font-semibold hover:text-blue-700">19XXX</a>
            </p>
        </form>
    </div>
</div>
